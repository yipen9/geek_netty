...
public class DirectByteBuffer extends MappedByteBuffer implements DirectBuffer {
    ...
    /**
     * {@link Cleaner#clean()} 将执行此Runnable
     */
    private static class Deallocator implements Runnable {
        ...
        public void run() {
            ...
            unsafe.freeMemory(address); //释放地址内存
            address = 0;
            Bits.unreserveMemory(size, capacity);   //重新修改reservedMemory
        }
    ...
    DirectByteBuffer(int cap) {                   // package-private
        ...
        super(-1, 0, cap, cap);
        boolean pa = VM.isDirectMemoryPageAligned();    //是否内存页对齐。
        ...
        //计算堆外内存的基地址
        if (pa && (base % ps != 0)) {   //如果内存对齐，并且起始地址不能整除每页的大小
            // Round up to page boundary
            address = base + ps - (base & (ps - 1));    //向上舍入到页面边界
        } else {
            address = base;
        }
        //在Cleaner 内部中通过一个列表，维护了一个针对每一个 directBuffer 的一个回收堆外内存的
        // 线程对象(Runnable)，回收操作是发生在 Cleaner 的 clean() 方法中。
        cleaner = Cleaner.create(this, new java.nio.DirectByteBuffer.Deallocator(base, size, cap));
        att = null;
