...
class Bits {                            // package-private
    ...
    static void reserveMemory(long size, int cap) {
        //sun.misc.VM.booted()以保证在application类加载器启动前不做任何事。booted()其实就是改了个状态,使isBooted()变为true
        if (!memoryLimitSet && VM.isBooted()) {
            maxMemory = VM.maxDirectMemory();   //最大堆外内存
            memoryLimitSet = true;              //限制设置内存
        }
        ...
        //等待释放内存的处理，最终调用的是Reference的waitForReferenceProcessing
    ...
    private static boolean tryReserveMemory(long size, int cap) {
        ...
        // aligned.
        long totalCap;
        while (cap <= maxMemory - (totalCap = totalCapacity.get())) {   //最大的 - 已经使用的总容量
            if (totalCapacity.compareAndSet(totalCap, totalCap + cap)) {    //更新totalCap
                reservedMemory.addAndGet(size); //更新预分配的size
                count.incrementAndGet();    //分配次数+1
                return true;
            }
        }
        return false;
